/**
 * social-links — the "Follow Us" icon row of the newsroom sidebar (source: component-socialLinks).
 * Authoring: one row, one cell, a <ul> of links (<li><a href="https://www.linkedin.com/…">LinkedIn</a></li> …).
 * Tier: reconstructive; the authored <ul> is MOVED into the icon-row wrapper (EW1). The FontAwesome
 * brand glyph is fixed per host (as the footer does); the authored link text stays in the DOM,
 * visually hidden.
 */
...
export default function decorate(block) {
  ...
  section.className = 'component-socialLinks';
  section.setAttribute('data-analytics-link-region', 'body');
  const icons = document.createElement('div');
  icons.className = 'social-icons align-left';
  if (ul) {
    ul.querySelectorAll('li > a').forEach((a) => {
      ...
      if (/^https?:/.test(a.getAttribute('href') || '') && !a.target) { a.target = '_blank'; a.rel = 'noopener'; }
    });
    icons.append(ul);
  }
